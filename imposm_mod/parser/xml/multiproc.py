...
from imposm_mod.parser.xml.parser import XMLParser
from imposm_mod.parser.util import setproctitle
# end of modification
...
class XMLMultiProcParser(object):
    ...
    # queue for bounding box added in the parameter list
    def __init__(self, pool_size, nodes_queue=None, ways_queue=None,
        relations_queue=None, coords_queue=None, bounds_queue=None, marshal_elem_data=False):
    # end of modification
        ...
        # modified for MoSP-GeoTool
        self.bounds_callback = bounds_queue.put if bounds_queue else None
        # end of modification
    ...
    def parse(self, stream):
        ...
        for _ in xrange(self.pool_size):
            proc = XMLParserProcess(self.mmap_pool, self.mmap_queue, nodes_callback=self.nodes_callback,
                ...
                bounds_callback=self.bounds_callback,
...
class XMLChunker(object):
    ...
    def _skip_header(self):
        for line in self.stream:
            ...
            # bounds and bound included as possible first line to parse
            if line.lstrip().startswith('<bounds ') or line.lstrip().startswith('<bound ') or line.lstrip().startswith('<node '):
            # end of modification    
...
if __name__ == '__main__':
    ...
    # modified for MoSP-GeoTool
    #bounds_queue = multiprocessing.JoinableQueue(128)
    # end of modification
    ...
    #    multiprocessing.Process(target=count_proc('ways', ways_queue)),
    #    multiprocessing.Process(target=count_proc('relations', relations_queue)),
    #    multiprocessing.Process(target=count_proc('bounds', bounds_queue))
    #]
    # end of modification
    for proc in procs:
        proc.start()
    ...
    #parser = XMLMultiProcParser(open(sys.argv[1]), 2, nodes_queue=nodes_queue,
    #    ways_queue=ways_queue, relations_queue=relations_queue, bounds_queue=bounds_queue)
    parser.start()
